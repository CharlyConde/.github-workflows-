...
# ==========================================
# GESTIÓN Y CARGA ROBUSTA DE DATOS
# ==========================================
...
def load_data():
    ...
    df_fallback = pd.DataFrame(columns=[
        "Fecha", "Jugador", "Vendedor", "Comprador", 
        "Precio Operación", "Valor Mercado", "Tipo"
    ])
    return df_fallback, csv_file
...
if df.empty:
    st.warning("⚠️ El archivo CSV está vacío o descargándose. Ejecuta 'extractor.py' localmente para poblar los datos completos.")
    st.stop()
...
with tab_kpis:
    ...
    if top_clau is not None:
        ...
        k4.metric("🔒 Mayor Clausulazo", "Sin datos", "Sin registros")
    ...
    if not df_caja.empty:
        ...
        st.info("No hay datos suficientes para calcular la caja.")
...
with tab_mercado:
    ...
    c2.metric("Sobrepuja Media Liga", fmt_pct(df_mercado["Sobreprecio (%)"].mean()) if not df_mercado.empty else "-")
    c3.metric("Sobrepuja Mediana Liga", fmt_pct(df_mercado["Sobreprecio (%)"].median()) if not df_mercado.empty else "-")
    ...
    if not df_mercado.empty:
        ...
        st.info("Sin registros de mercado suficientes.")
...
with tab_rivales:
    ...
    if todos_managers:
        ...
        st.info("No hay mánagers registrados en el sistema.")
